...
// Package Declaration (SystemVerilog-2017)
package counter_pkg;
  parameter WIDTH = 4;
  ...
  class CounterTransaction;
    ...
    rand bit [1:0] state;
    constraint valid_count { soft count <= `MAX_COUNT; }
    function new();
      count = 0;
      state = IDLE;
    endfunction
...
// Interface Declaration (SystemVerilog-2017)
interface counter_if (
  ...
  // Let Construct (SystemVerilog-2017, fails 2012 parser)
  let valid_count = count <= `MAX_COUNT;
endinterface

// Module Declaration: Counter (SystemVerilog-2017)
module counter import counter_pkg::*; (
  ...
  // Let Construct (SystemVerilog-2017, fails 2012 parser)
  let is_valid = internal_count <= `MAX_COUNT;
  ...
  // Concurrent Assertion using Let
  assert property (@(posedge ifc.clk) is_valid)
...
// Program Declaration: Testbench (SystemVerilog-2017)
program tb import counter_pkg::*; (
  ...
  initial begin
    ...
    repeat (20) begin
      ...
      tx.display();
      if (!ifc.valid_count)
        $display("Invalid count detected");
    end
    $finish;
  end
endprogram